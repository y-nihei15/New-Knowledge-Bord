    ...
    async function postJSON(url, payload, { timeoutMs = 15000 } = {}) {
      ...
      try {
        ...
        if (raw) {
          ...
          catch { throw new Error(`Invalid JSON (HTTP ${res.status}): ${raw.slice(0,200)}`); }
        }
        if (!res.ok) {
          if (res.status === 401) location.replace(<?= json_encode($loginScreen, JSON_UNESCAPED_SLASHES) ?>);
