...
function require_auth(): array
{
    ...
    try {
        ...
        // よくあるラップ形を平坦化
        if (isset($payload['claims'])  && is_array($payload['claims']))  $payload = $payload['claims'];
        if (isset($payload['payload']) && is_array($payload['payload'])) $payload = $payload['payload'];

        // jti / jwt_id 両対応
        $jti = $payload['jti'] ?? ($payload['jwt_id'] ?? '');
        if ($jti === '') _auth_fail('invalid_token', 'Missing jti in token', 401, $realm);
        $payload['jti'] = $payload['jwt_id'] = $jti;

        // ID系も相互補完（どれか1つあればOKに）
        $userId = $payload['sub'] ?? ($payload['user_id'] ?? ($payload['account_id'] ?? null));
        if ($userId === null) _auth_fail('invalid_token', 'Missing subject (sub/user_id/account_id)', 401, $realm);
        $payload['sub']        = $payload['sub']        ?? $userId;
        $payload['user_id']    = $payload['user_id']    ?? $userId;
        $payload['account_id'] = $payload['account_id'] ?? $userId;
    } catch (Throwable $e) {
